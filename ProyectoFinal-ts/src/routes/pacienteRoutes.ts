...
export async function pacienteRoutes(req: IncomingMessage, res: ServerResponse, partes: string[]) {
    const id = partes[0] ? Number(partes[0]) : undefined;
    try {
        if (req.method === "GET" && id === undefined) return controller.listar(req, res);
        if (req.method === "GET" && id !== undefined) return controller.buscarPorId(req, res, id);
