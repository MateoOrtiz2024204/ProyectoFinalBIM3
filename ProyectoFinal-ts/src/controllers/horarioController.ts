...
export class HorarioController {
    ...
    async buscarPorMedico(_req: IncomingMessage, res: ServerResponse, idMedico: number) {
        try {
            enviarJSON(res, 200, await service.buscarPorMedico(idMedico));
        } catch (e: any) {
    ...
    async agregar(req: IncomingMessage, res: ServerResponse) {
        try {
            ...
            enviarJSON(res, 201, { mensaje: "Horario agregado" });
        } catch (e: any) {
            enviarError(res, 400, e.message);
        }
    }
    ...
    async actualizar(req: IncomingMessage, res: ServerResponse, id: number) {
        try {
            ...
            enviarJSON(res, 200, { mensaje: "Horario actualizado" });
        } catch (e: any) {
            enviarError(res, 400, e.message);
        }
    }
    ...
    async eliminar(_req: IncomingMessage, res: ServerResponse, id: number) {
        try {
            ...
            enviarJSON(res, 200, { mensaje: "Horario eliminado" });
        } catch (e: any) {
            enviarError(res, 400, e.message);
        }
    }
